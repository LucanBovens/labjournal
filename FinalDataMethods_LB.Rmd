...
## RStudio
    [Description of which packages are used and for what purpose] needed? \
...
## Model estimation
     AANPASSEN!! The RSiena analysis will actually consist of multiple models. At first, the analysis will start with a null model. This null model will only include the density and reciprocity structural effects, as this is a standard setting within RSiena. Model 1 will contain the necessary structural effects for the analysis, such as xxx and xxx. Then, Model 2 contains the covariate effect of prestige (plus network statistic?). In Model 3 the covariate effect of gender will be added, to serve as a control variable. Model 4 then functions as the full model, in which all covariate effects and necessary structural effects are added. Finally, goodness of fit tests will be applied to the models to check whether the xxx. \
    
 
